...
def afficher_resultats(suites_trouvees):
    ...
    for cle, suites in suites_trouvees.items():
        if cle.startswith('Verticale_'):
            ...
            date, type_tirage = cle.split('_', 1)
        ...
        for i, suite in enumerate(suites):
            ...
            if suite['type_suite'] == 'constante':
                ...
                # Afficher si la suite est complète et les éléments manquants
                if 'est_complete' in suite:
                    status = "complète" if suite['est_complete'] else "incomplète"
                    print(f"  Statut: Suite {status}")

                    if not suite['est_complete'] and 'elements_manquants' in suite:
                        print(f"  Éléments manquants: {suite['elements_manquants']}")
            else:
...
if __name__ == "__main__":
    ...
    fichier = "./api/uploads/formatted_lottery_results.csv"
    ...
    types_tirage = None
    date_debut = "05/10/2020"
    date_fin = "11/10/2020"
    direction = "vertical"
    difference_constante = True
    respecter_ordre_apparition = False
    longueur_suite_filtre = 4
    verifier_completion = True  # Nouveau paramètre pour vérifier si une suite est complète
    valeur_min = 1  # Valeur minimale pour les numéros
    valeur_max = 90  # Valeur maximale pour les numéros

    # Analyse et affichage des résultats
    resultats = analyser_suites_arithmetiques(
        fichier, sens_lecture, colonnes, respecter_positions,
        types_tirage, date_debut, date_fin, direction,
        difference_constante, respecter_ordre_apparition, longueur_suite_filtre,
        verifier_completion, valeur_min, valeur_max
    )
