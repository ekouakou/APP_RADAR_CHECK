...
# Fonction d'analyse principale (conservée depuis le code original)
...
def analyser_meme_ligne_progressions(
    ...
    for idx, row in df.iterrows():
        ...
        while i < len(valeurs) - 1:
            ...
            j = i + 1
            while j < len(valeurs) and not pd.isna(valeurs[j]) and valeurs[j] - valeurs[j - 1] == diff_constante:
                sequence_courante.append(valeurs[j])
                j += 1
...
# Endpoint pour recevoir et analyser un fichier CSV
...
def analyser():
    ...
    type_tirage = request.form.get('type_tirage')
    longueur_min = int(request.form.get('longueur_min', 3))
    type_analyse = request.form.get('type_analyse')

    # Convertir les paramètres booléens
    respecter_position = request.form.get('respecter_position', 'false').lower() == 'true'
    analyser_meme_ligne = request.form.get('analyser_meme_ligne', 'false').lower() == 'true'
    fusionner_num_machine = request.form.get('fusionner_num_machine', 'false').lower() == 'true'
    utiliser_longueur_min = request.form.get('utiliser_longueur_min', 'true').lower() == 'true'
    reverse_order = request.form.get('reverse_order', 'false').lower() == 'true'
    ...
    try:
        ...
        # Si les résultats sont un message d'erreur (chaîne de caractères)
...
# Route pour tester si le serveur est en fonctionnement
@app.route('/api/test', methods=['GET'])
def test():
    return jsonify({'status': 'success', 'message': 'Le serveur fonctionne correctement'}), 200


# Liste des colonnes attendues dans le CSV
